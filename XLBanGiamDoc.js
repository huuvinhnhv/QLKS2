...
function DocDuLieuPhong(username) {
    ...
    const nhanVien = duLieuKhachSan.banGiamDoc.find(nv => nv.maNV === username);
...
function LuuDuLieuKhachSan(data) {
    const DuongDan = PATH.join(ThuKhachSan, "KhachSan.json");
    const ChuoiJSON = JSON.stringify(data, null, 2); // định dạng đẹp (indent = 2)
    FS.writeFileSync(DuongDan, ChuoiJSON, "utf-8");
}
function CapNhatGiaThueLoaiPhongMoi(tenLoaiPhongCanSua, giaThueMoi) {
    const danhSachFile = FS.readdirSync(ThuMucPhong);
    let demPhongDaCapNhat = 0;

    danhSachFile.forEach((tenFile) => {
        if (tenFile.endsWith(".json")) {
            const duongDan = PATH.join(ThuMucPhong, tenFile);
            const chuoiJSON = FS.readFileSync(duongDan, "utf-8");
            const phong = JSON.parse(chuoiJSON);

            if (phong.loaiPhong?.toLowerCase() === tenLoaiPhongCanSua.toLowerCase()) {
                phong.donGia = giaThueMoi;
                FS.writeFileSync(duongDan, JSON.stringify(phong, null, 2), "utf-8");
                demPhongDaCapNhat++;
            }
        }
    });

    return demPhongDaCapNhat; // trả về số lượng phòng đã cập nhật
}
...
function LayDanhSachLoaiPhong() {
    return DocDuLieuKhachSan().loaiPhong;
}
// Xử lý giao diện
...
<nav class="fixed-top navbar navbar-expand-lg navbar-dark bg-primary px-3" style="min-height: 8vh;">
    ...
    <form class="form-inline mx-1" method="get" action="/timkiem">
     ...
     <a  class="btn btn-light" href="/danhsachloaiphong">Thay đổi giá</a>
...
<form action="/thongkethuthang" method="get" class="d-inline-block mr-2">
    <div class="d-flex align-items-center">
      <label class="text-white mx-2">Tháng năm:</label>
        ...
        <button type="submit" class="btn btn-light">TK tháng</button>
    </div>
</form>
...
<form action="/thongkethunam" method="get" class="d-inline-block">
    <div class="d-flex align-items-center">
       <label class="text-white mx-2">Năm:</label>
        ...
        <button type="submit" class="btn btn-light">TK năm</button>
    </div>
</form>
...
function TaoHtmlSuaLoaiPhong(loaiPhong) {
    const maLoaiPhong = loaiPhong.maPhong || 'Không rõ';
    const tenLoaiPhong = loaiPhong.tenLoaiPhong || 'Không rõ';
    const donGia = loaiPhong.donGia || 0;
    const sucChua = loaiPhong.sucChua || 'Không rõ';
    const tienNghi = loaiPhong.tienNghi || 'Không rõ';

    return `
    <div class="container mt-4">
        <div class="card border-info shadow-sm">
            <div class="card-header bg-info text-white text-center">
                <h4 class="mb-0">Chỉnh sửa đơn giá phòng: ${tenLoaiPhong}</h4>
            </div>
            <div class="card-body">
                <form method="POST" action="/luugia">
                    <div class="form-group">
                        <label><strong>Mã loại phòng:</strong></label>
                        <input type="text" class="form-control"  value="${maLoaiPhong}" readonly>
                    </div>
                    <div class="form-group">
                        <label><strong>Tên loại phòng:</strong></label>
                        <input type="text" class="form-control" name="maLoaiPhong" value="${tenLoaiPhong}" readonly>
                    </div>
                    <div class="form-group">
                        <label><strong>Sức chứa:</strong></label>
                        <input type="text" class="form-control" value="${sucChua}" readonly>
                    </div>
                    <div class="form-group">
                        <label><strong>Tiện nghi:</strong></label>
                        <textarea class="form-control" rows="2" readonly>${tienNghi}</textarea>
                    </div>
                    <div class="form-group">
                        <label><strong>Đơn giá mới (VND):</strong></label>
                        <input type="number" class="form-control" name="donGia" value="${donGia}" required min="0">
                    </div>
...
function TaoHtmlMenuLoaiPhong(loaiPhong) {
    const maLoaiPhong = loaiPhong.maPhong || 'Không rõ';
    const tenLoaiPhong = loaiPhong.tenLoaiPhong || 'Không rõ';
    const donGia = loaiPhong.donGia?.toLocaleString('vi-VN') || 'Không rõ';
    const sucChua = loaiPhong.sucChua || 'Không rõ';
    const tienNghi = loaiPhong.tienNghi || 'Không rõ';
    return `
    <div class="col-sm-6 col-md-4 col-lg-3 mb-4">
        <div class="card h-100 border-info shadow-sm">
            <div class="card-header bg-info text-white text-center">
                <strong>${tenLoaiPhong}</strong>
            </div>
            <div class="card-body">
                <p class="card-text mb-2">
                    <strong>Mã loại phòng:</strong> ${maLoaiPhong}
                </p>
                <p class="card-text mb-2">
                    <strong>Đơn giá: ${donGia} VND</strong>
                </p>
                <p class="card-text mb-2">
                    <strong>Sức chứa:</strong> ${sucChua} người
                </p>
                <p class="card-text">
                    <strong>Tiện nghi:</strong> ${tienNghi}
                </p>
            </div>
            <div class="card-footer bg-white border-top-0 d-flex justify-content-center">
                <a href="/thaydoigiathue/${maLoaiPhong}" class="btn btn-outline-primary btn-sm">
                    <i class="fa fa-edit"></i> Sửa giá
                </a>
            </div>
        </div>
    </div>
...
function TaoHtmlMenuDanhSachLoaiPhong(danhSachLoaiPhong) {
    let allCardsHtml = '';
    for (const loaiPhong of danhSachLoaiPhong) {
        allCardsHtml += TaoHtmlMenuLoaiPhong(loaiPhong);
    }
    const finalHtml = `
          <h3 class="text-left pt-2 pb-2">Danh sách loại phòng:</h3>
    <div class="row">${allCardsHtml}</div>`;
    return finalHtml;
}
...
function LuuGiaThueMoi(tenLoaiPhongMoi, giaThueMoi) {
    const khachSan = DocDuLieuKhachSan()
    // Tìm loại phòng cần cập nhật
    const loaiPhong = khachSan.loaiPhong.find(lp => lp.tenLoaiPhong.toLowerCase() === tenLoaiPhongMoi.toLowerCase());
    if (!loaiPhong) {
        console.log(`Không tìm thấy loại phòng với mã ${tenLoaiPhongMoi}`);
        return;
    }
    // Cập nhật giá thuê mới trong file khách sạn
    loaiPhong.donGia = giaThueMoi;
    LuuDuLieuKhachSan(khachSan);

    CapNhatGiaThueLoaiPhongMoi(tenLoaiPhongMoi, giaThueMoi);
}
...
module.exports.LayDanhSachLoaiPhong = LayDanhSachLoaiPhong;
module.exports.TaoHtmlMenuDanhSachLoaiPhong = TaoHtmlMenuDanhSachLoaiPhong;
module.exports.TaoHtmlSuaLoaiPhong = TaoHtmlSuaLoaiPhong;
module.exports.LuuDuLieuKhachSan = LuuDuLieuKhachSan;
module.exports.CapNhatGiaThueLoaiPhongMoi = CapNhatGiaThueLoaiPhongMoi;
module.exports.LuuGiaThueMoi = LuuGiaThueMoi;